...
final myCommunityListProvider =
    StateNotifierProvider<MyCommunityListStateNotifier, PaginationBase>((ref) {
  final Dio dio = Dio();
  ...
  final notifier = MyCommunityListStateNotifier(
      repository: repository, userUUID: authModel.userUUID ?? '');

  return notifier;
});

class MyCommunityListStateNotifier extends StateNotifier<PaginationBase> {
  final MemberRepository repository;
  final String userUUID;

  MyCommunityListStateNotifier(
      {required this.repository, required this.userUUID})
  ...
  Future<void> paginate({
    ...
    try {
      ...
      if (fetchMore) {
        final pState = state as Pagination<CommunityModel>;

        state = PaginationFetchingMore<CommunityModel>(
      ...
      final res = await repository.paginateCommunities(
