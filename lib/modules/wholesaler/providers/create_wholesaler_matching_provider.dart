...
final createwholesalerMatchingProvider = StateNotifierProvider.autoDispose<CreatewholesalerMatchingStateNotifier, ServerStatusBase>((ref) {
  ...
  final notifier = CreatewholesalerMatchingStateNotifier(repository: repository, userUUID: userUUID);

  return notifier;
});

class CreatewholesalerMatchingStateNotifier extends StateNotifier<ServerStatusBase>{
  final String userUUID;
  final MatchingRepository repository;

  CreatewholesalerMatchingStateNotifier({required this.userUUID,required this.repository}): super(ServerStatusInitial());

  Future<void> onMatch({required int wholesalerId}) async {
    state = ServerStatusLoading();
    try {
      final CreateMatchingWSModel data = CreateMatchingWSModel(
          userUUID: userUUID,
          wholesalerId: wholesalerId,
          matchingWSStatus: MatchingWSStatus.MATCHING_STANDBY,
          activate: true
      );

      final res = await repository.createWSMatching(data: data);
